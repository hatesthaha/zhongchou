 ...
 * @since 2.0
 */
class GridViewechart extends BaseListView
{
    ...
    public function renderTableBody()
    {
        ...
        $pro_n_nums = [];//关注数
        $pro_search_nums = [];//搜索数
        $echat_str = '<div id="main" style="height:400px;"></div>
...
$('Document').ready(function(){
    ...
    myChart.setOption(option);
        ...
        foreach ($models as $index => $model) {
            ...
            $fanhuizhi =  $this->renderTableRow($model, $key, $index);
            $rows[] = $fanhuizhi;
            //print_r($fanhuizhi);echo "<br>";
            $pro_names[] = $fanhuizhi[1];
            $pro_s_nums[] = $fanhuizhi[3];
            $pro_n_nums[] = $fanhuizhi[2];
            $pro_search_nums[] = $fanhuizhi[4];
        ...
        $echat_str = sprintf(
            $echat_str, 
            preg_replace(['@<td>@siu', '@</td>@siu', '@<span class="not-set">@siu', '@</span>@siu'], ['"', '"'], implode(',', $pro_names)),
            preg_replace(['@<td>@siu', '@</td>@siu', '@<span class="not-set">@siu', '@</span>@siu'], ['"', '"'], implode(',', $pro_n_nums)),
            preg_replace(['@<td>@siu', '@</td>@siu', '@<span class="not-set">@siu', '@</span>@siu'], ['"', '"'], implode(',', $pro_s_nums)),
            preg_replace(['@<td>@siu', '@</td>@siu', '@<span class="not-set">@siu', '@</span>@siu'], ['"', '"'], implode(',', $pro_search_nums))
        ...
        if (empty($rows)) {
            ...
            //return "<tbody>\n" . implode("\n", $rows) . "\n</tbody>";
            return "<tbody>\n<tr rowspan=\"$rowspan\"><td colspan=\"$colspan\">" .$echat_str. "</td></tr>\n</tbody>";
