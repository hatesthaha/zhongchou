...
$jujuekefu = Setting::find()->where('code=:code', [':code'=>'jujuekefu'])->one();
?>
...
<head>
	...
	<meta content="修改项目" name="Keywords">
...
<body>
	...
	<div id="body_h">
		...
		<section class="modular34 bgeee">
			<div class="modular34con1">
				<a class="modular34con1word" href="/address/chooseaddress?xiugai=true&pid=<?= $proinfo['id'] ?>">
					<div>
						<div>
						<?php if(!empty($noadd)): ?>
							请选择收货地址
						<?php else: ?>
							<p class="clearfix text-r"><span class="float-left">收货人：<?php if(!empty($address['province'])){echo $address['username'];}else{echo $address['name'] ;} ?></span><span class="modular17-cond"><?= $address['phone']; ?></span></p>
							<p class="modular17-cond">收货地址：<?php if(!empty($address['province'])){echo $address['province'].$address['city'].$address['county'].$address['address']; } else {echo $address['address'];}?></p>
						<?php endif ?>						</div>
					</div>
					<p><i class="icon-angle-right"></i></p>
				</a>
			</div>
			<form action="/project/xiugai" method="POST" id='form'>
				...
				<div class="modular34con3">
					<div class="modular7-con">
						<div class="twoelement modular7-con2">
							...
							<input id="name" type="text" class="changeelement" name="projectname" placeholder='标题' value="<?= $proinfo['name']?>">
							<i class="rigd rigdx icon-remove hide"></i>
						</div>
						<div class="twoelement modular7-con2">
							<p class="fixedelement2">参考价格</p>
							<input id="targetmoney" type="text" class="changeelement" name="chouzje" placeholder='您可发起100~<?= $userinfo['tmoney']?>元的项目' value="<?= $proinfo['target_money']?>">
							
							<span>元</span>
						</div>
						<div class="twoelement modular7-con2">
							<p class="fixedelement2">参考链接</p>
							<input id="pro_href" type="url" class="changeelement" name="pro_href" placeholder='商品链接必须以‘http’开头' value="<?= $proinfo['pro_href']?>">
						</div>
					...
					<div id="mould-34list1" class="xjclass7 modular34con4con hide xjclass2">
						<div class="modular34con4contop">
							...
							<p class="changeelement">
								<textarea id="xjtextarea" placeholder='请填写筹款目的，有助于通过审核哦！' name="chouzmd" rows="3"><?= $proinfo['content']?></textarea> 
								<span><i></i></span>
							</p>
						</div>
				...
				<p class="form-btn clearfix">
					<a class="float-left bghui-wc" href="/project/cancel?pid=<?= $proinfo['id'] ?>">取消发起</a>
					<a id="tijiao-mask" class="float-right bgfeb528-wf"><?php if ($proinfo['shenhe']==0){echo "提交修改";}elseif($proinfo['shenhe']==2){echo "重新提交";} ?></a>
				</p>
				<?php if($proinfo['shenhe']==2 && !empty($proinfo['jujueyuanyin'])){ ?>
				<p class="form-btn clearfix" style="color: #a94442;"><span >*审核失败原因：</span><br><?php echo $proinfo['jujueyuanyin']; ?></p><br><br>
				<p class="nextlinkpage">
						<a class="bgfeb528-wf" href="tel:<?= empty($jujuekefu['value'])?"":$jujuekefu['value']; ?>">拨打客服电话咨询</a>
					</p>
				<?php }?>
			</div>
			</form>
	...
	$(document).ready(function(){
		...
		$("#tijiao-mask").click(function(){
			...
			var address = $(".modular17-cond").html();
			if (address !=""){
				aid = true;
			}
		...
		// http://b.mashort.cn/h.o2ogs?cv=AADubdzc&sm=b2160f 
			...
			if(!aid){
				...
				$.post("/project/pdmoney",{money:targetmoney},function(result){
					if (result) {
						...
						$("input[name='aid']").val(<?php if(!empty($_GET['aid'])){echo $_GET['aid'];}else {echo 0;}?>);
						$("#form").submit();
					}else{
						changemonety = false;
